...
def switch(input, cases, results):
...
def noise_type_str(noise_type):
	return switch(noise_type, [1,2,3], ['UAR','CYC','MAJ'])

def alg_str(alg_name):
	return switch(alg_name, ALG_NAMES_SUM, ALG_NAMES_LATEX)

def alg_str_compatible(alg_name):
	return switch(alg_name, ALG_NAMES_SUM, ALG_NAMES_COMPATIBLE)

def alg_color_style(alg_name):
	return switch(alg_name, ALG_NAMES_SUM, ALG_COLORS_STYLES)

def alg_index(alg_name):
	return switch(alg_name, ALG_NAMES_SUM, ALG_ORDERS)
